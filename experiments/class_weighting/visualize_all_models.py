#!/usr/bin/env python3
"""
Visualize predictions from ALL class-weighting checkpoints.
...
def ckpt_to_model_name(ckpt_path):
    ...
    e.g. 'cw_balanced_softmax_tau_1.0_20260211_024949_best.pt'
    ...
    fname = re.sub(r'^cw_', '', fname)              # drop cw_ prefix
    return fname
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize ALL class-weighting model predictions')
    ...
    # De-duplicate: if multiple dates exist for the same model, keep latest
    ckpt_by_model = {}
    for ckpt in all_ckpts:
        name = ckpt_to_model_name(ckpt)
        ckpt_by_model[name] = ckpt  # sorted order means last = latest
    ...
    print("Phase 1: Caching validation samples (shared across all models)...")
