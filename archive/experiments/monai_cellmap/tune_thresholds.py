...
def tune_thresholds(args):
    ...
    for file_entry in tqdm(val_files, desc="Volumes"):
        ...
        for c, class_name in enumerate(CLASS_NAMES):
            ...
            # Reconstruct nuc (c=13) from sub-nuclear classes since it's
            # no longer stored in single-label NIfTI
            if c == 13:  # nuc
                _NUC_SUB_IDS = [21, 22, 23, 24, 25, 26, 27, 33, 34, 35]
                for sid in _NUC_SUB_IDS:
                    gt_binary = np.maximum(gt_binary, (gt_data == sid).astype(np.float32))
