...
class BalancedSoftmaxTverskyLoss(nn.Module):
    ...
    **Mask-supervised reconstruction (masksup_r0.3)**:
    During training, randomly masks `masksup_ratio` (30%) of annotated voxels
    and adds a weighted reconstruction Tversky loss on the masked subset.
    This forces the model to predict masked voxels from context, improving
    generalization. Winner of masking_strategies experiment after foreground fix:
        masksup_r0.3: 0.5711 eval Dice (+12% vs no_mask 0.511)
    Set masksup_ratio=0.0 to disable (e.g. for 3D where it's less applicable).
    ...
    Args:
        ...
        masksup_ratio: Fraction of annotated voxels to mask for reconstruction.
            Default 0.0 (disabled). Set to 0.3 for 2D training.
        masksup_recon_weight: Weight for the reconstruction loss branch.
            Default 0.5.
    """
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        def _tversky_from_weights(w):
            """Compute per-channel Tversky loss given spatial weight mask w."""
            tp = (w * pred * target).sum(dim=spatial_dims)                  # (B, C)
            fp = (w * pred * (1.0 - target)).sum(dim=spatial_dims)          # (B, C)
            fn = (w * (1.0 - pred) * target).sum(dim=spatial_dims)          # (B, C)
            denom = tp + self.alpha * fp + self.beta * fn + self.smooth
            tversky = (tp + self.smooth) / denom.clamp(min=self.smooth)
            return 1.0 - tversky  # (B, C)

        # --- Mask-supervised reconstruction (masksup) ---
        # During training, randomly mask `masksup_ratio` of annotated voxels
        # and compute a separate reconstruction Tversky on the masked subset.
        # This forces context-based learning (+12% Dice in 2D experiments).
        if self.training and self.masksup_ratio > 0:
            # Random mask: True = masked for reconstruction, False = visible
            rand = torch.rand_like(spatial_w)
            # Only mask voxels that have non-zero spatial weight (i.e. annotated)
            recon_mask = (spatial_w > 0) & (rand < self.masksup_ratio)
            visible_w = spatial_w * (~recon_mask).float()
            recon_w = spatial_w * recon_mask.float()

            # Standard loss on visible annotated voxels
            per_channel_loss = _tversky_from_weights(visible_w)

            # Reconstruction loss on masked voxels
            if recon_mask.any():
...
def build_partial_annotation_loss(
    ...
    Args:
        ...
        masksup_ratio: Fraction of annotated voxels to mask for reconstruction
            loss. 0.0 = disabled, 0.3 = recommended for 2D training.
        masksup_recon_weight: Weight for reconstruction branch (default 0.5).
